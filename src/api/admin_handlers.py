...
async def create_user(user: AdminUserCreateSchema, request: Request, session: AsyncSession = Depends(get_session),
...
@limiter.limit("10/minute")
async def update_user(user: AdminUserUpdateSchema,
                      request: Request,
                      session: AsyncSession = Depends(get_session),
                      token: TokenData = Depends(require_admin)):
